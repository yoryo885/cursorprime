"""Checkpoint del meta-creador por slug."""
...
from src.config import CHECKPOINT_ENABLED, borrador_meta, load_json, save_json
...
class Checkpoint:
    ...
    @property
    def path(self):
        return borrador_meta(self.slug) / ".checkpoint.json"

    @classmethod
    def load(cls, slug: str) -> "Checkpoint":
        path = borrador_meta(slug) / ".checkpoint.json"
        data = load_json(path, {})
        return cls(
            slug=slug,
            fase=data.get("fase", "diseno"),
